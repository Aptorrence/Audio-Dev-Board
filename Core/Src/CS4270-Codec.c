...
uint8_t CS4270_REG_CONFIG_SETTINGS[8] = {CS4270_DEVICEID,	//Device ID
										 ...
										 0x09,				//ADC/DAC (disable HPF, I2
										 0x00,				//Transition (Independent volume controls)
										 0x00,				//Mute right channel (for now we stayin mono)
										 0x00,				//DAC A Vol (0db)
										 0x00};				//DAC B Vol (mute (-127.5db))

static I2C_HandleTypeDef * CS4270_I2C_HANDLE;
static GPIO_TypeDef * CS4270_NRST_GPIO_PORT;
static uint16_t CS4270_NRST_GPIO_PIN;

uint8_t CS4270_Init(I2C_HandleTypeDef * i2cInstance, GPIO_TypeDef * nrstPort, uint16_t nrstPin) {

	CS4270_I2C_HANDLE = i2cInstance;
	CS4270_NRST_GPIO_PORT = nrstPort;
	CS4270_NRST_GPIO_PIN = nrstPin;
	...
	// put CODEC in power down mode before adjusting configuartion (set freeze bit, power down ADC, DAC, enter I2Cstatus
	...
	if( (i2cStatus != HAL_OK) || ((deviceID & 0xF0) != CS4270_DEVICEID)){
...
// Set configuration registers
	...
	for (regIndex = CS4270_REG_DACBVOLCONTROL; regIndex >= CS4270_REG_POWERCONTROL; regIndex--){
		...
		// Write 'default' value to register (Note: not checking reserved bits - default values are 0 in datasheet
